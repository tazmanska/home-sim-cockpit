...
namespace FsuipcSdk
{
	...
	public class Fsuipc 
	{
		...
		public bool FSUIPC_Open(int dwFSReq, ref int dwResult) 
		{
			...
			szName = FS6IPC_MSGNAME1 + ":" + System.Threading.Thread.CurrentThread.ManagedThreadId.ToString("X") /* GetCurrentProcessId().ToString("x") */ + ":" + nTry.ToString("x") + Convert.ToChar(0).ToString();
			if (szName.Length > 23) 
			{    //24 chars max
